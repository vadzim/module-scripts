...
const makeCommands = () => {
	const commands = {
		async init() {
			const pkgfn = await packageJsonFile()
			const pkg = JSON.parse(await fs.readFile(pkgfn))
			if (!hasDependency(pkg, name)) {
				await installDependency(pkgfn, name)
			}
			await changeJsonFile(pkgfn, initPackageJson)
		},

		async cleanup() {
			await changeJsonFile(await packageJsonFile(), cleanupPackageJson)
		},
